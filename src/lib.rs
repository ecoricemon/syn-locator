...
/// Global location storage that can be accessed across threads.
static LOCATOR: LazyLock<Mutex<Locator>> = LazyLock::new(|| Mutex::new(Locator::new()));

thread_local! {
    static EN_TL_LOCATOR: Cell<bool> = const { Cell::new(false) };

    /// Per-thread global location storage.
    static TL_LOCATOR: RefCell<Locator> = RefCell::new(Locator::new());
}

pub fn enable_thread_local(en: bool) {
    EN_TL_LOCATOR.replace(en);
}

pub fn is_located(file_path: &str) -> bool {
    if EN_TL_LOCATOR.get() {
        TL_LOCATOR.with(|locator| locator.borrow().contains_file(file_path))
    } else {
        LOCATOR.lock().unwrap().contains_file(file_path)
    }
}
...
pub fn clear() {
    if EN_TL_LOCATOR.get() {
        TL_LOCATOR.with_borrow_mut(|locator| locator.clear());
    } else {
...
pub enum LocatorGuard<'a> {
    Mutex(MutexGuard<'a, Locator>),
    RefMut(RefMut<'a, Locator>),
}

impl<'a> ops::Deref for LocatorGuard<'a> {
    type Target = Locator;
...
impl ops::DerefMut for LocatorGuard<'_> {
...
pub trait LocateEntry: Locate {
    fn locate_as_entry(self: Pin<&Self>, file_path: &str, code: &str) -> Result<()> {
        let loc = self.location(file_path, code)?;

        if EN_TL_LOCATOR.get() {
            TL_LOCATOR.with(|locator| {
                let mut locator = LocatorGuard::RefMut(locator.borrow_mut());
                let Some(code) = locator.filtered_code_ptr(file_path) else {
                    drop(locator);
                    panic!("failed to find `{file_path}`");
                };

                // Safety: Locating doesn't modify code itself. Therefore, the address
                // and its content will never change until end of this block.
                unsafe {
                    let code = code.as_ref().unwrap_unchecked();
                    self.locate(&mut locator, loc.file_path, code, 0);
                }
            });
        } else {
            let mut locator = LocatorGuard::Mutex(LOCATOR.lock().unwrap());
            let Some(code) = locator.filtered_code_ptr(file_path) else {
                drop(locator);
                panic!("failed to find `{file_path}`");
            };
    ...
    #[doc(hidden)]
    fn location(self: Pin<&Self>, file_path: &str, code: &str) -> Result<Location> {
        let code: Box<str> = code.into();

        if EN_TL_LOCATOR.get() {
            TL_LOCATOR.with_borrow_mut(|locator| locator.insert_file(&*self, file_path, code))
        } else {
            LOCATOR.lock().unwrap().insert_file(&*self, file_path, code)
        }
    }
}
...
pub trait Locate: Any {
    ...
    ///
    /// This method takes a mutex guard for the global location storage. Do not
    /// call methods shown below which try to lock the storage.
    /// - [`Locate::location`]
    /// - [`Locate::location_message`]
    /// - [`Locate::code`]
    ///
    /// But you have some alternatives like,
    /// - [`Locate::_location`]
    fn find_loc(
        &self,
    ...
    fn relocate(&self, locator: &mut LocatorGuard, loc: Location) {
    ...
    /// This method tries to lock the global location storage. If you have a
    /// mutex guard for it already, do not call this method.
    fn location(&self) -> Location {
        if EN_TL_LOCATOR.get() {
            TL_LOCATOR
                .with_borrow_mut(|locator| locator.get_location(self))
                .unwrap_or_else(|| {
                    panic!(
            ...
            let Some(loc) = locator.get_location(self) else {
                drop(locator);
                panic!(
                    "failed to find the location of `{}`. did you forget `Locate::locate`?",
                    any::type_name_of_val(self)
    ...
    fn _location(&self, locator: &LocatorGuard) -> Location {
    ...
    fn location_message(&self) -> String {
        if EN_TL_LOCATOR.get() {
            ...
            let path = loc.file_path;
            let Some(code) = locator.get_original_code(path) else {
                drop(locator);
                panic();
            };
...
pub trait LocateGroup {
    ...
    fn relocate_as_group(&self, locator: &mut LocatorGuard, loc: Location);
}
...
impl LocateGroup for () {
    ...
    fn relocate_as_group(&self, _: &mut LocatorGuard, _: Location) {}
}
...
pub struct Locator {
    files: HashMap<&'static str, Content>,
...
impl Locator {
    ...
    fn insert_file<T: Any + ?Sized>(
        ...
        fn inner(
            ...
            if this.contains_file(file_path) {
                return Err(
            ...
            this.files.insert(file_path, Content::new(code));
    ...
    fn get_original_code(&self, file_path: &str) -> Option<&str> {
        self.files
            .get(file_path)
            .map(|file| (*file.original_code).as_ref())
    }

    fn filtered_code_ptr(&self, file_path: &str) -> Option<*const str> {
...
struct Content {
    original_code: Pin<Box<str>>,
    filtered_code: Pin<Box<str>>,
}
...
impl Content {
    ...
    /// token matching.
    fn remove_non_tokens(code: &str) -> Pin<Box<str>> {
        ...
        let s: Box<str> = re
            .replace_all(code, |caps: &Captures| " ".repeat(caps[0].len()))
            .into();
        Box::into_pin(s)
    }
}
